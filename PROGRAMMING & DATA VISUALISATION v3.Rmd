...
```
# Step 6: There are some nulls in likes. I'd like to count how many of these likes are null.
```{sql, connection = con}
select count(*) from imdb_scores where likes is null
```
# Step 7: I'd like to see how many rows imdb_likes contains.
```{sql, connection = con}
select count(*) from imdb_scores
```

# Step 8: In fact there are several nulls in "likes", I want to check if there's any in "score".
```{sql, connection = con}
...
```
# Step 16: Since there are so many null values in imdb_likes, i want to exclude it from our query.
```{sql, connection = con, results='markup', output.var = "query_results"}
...
# Step 29: Now I want to analyze genres best to worst and worst to best. Creating a new dataframe with the average score and genre(s).
```{r}
